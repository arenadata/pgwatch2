...
if [ -n "$GIT_TIME" -a -n "$GIT_HASH" ] ; then
  ...
  go build -gcflags "all=-N -l" -ldflags "-X main.commit=$GIT_HASH -X main.date=$GIT_TIME" pgwatch2.go prom.go patroni.go logparse.go
    ...
    go build -gcflags "all=-N -l" -ldflags "-X main.commit=`cat build_git_version.txt`" pgwatch2.go prom.go patroni.go logparse.go
else
  ...
  if [ -n "$git_on_path" -a -f pgwatch2.go ] ; then
    ...
    go build -gcflags "all=-N -l" -ldflags "-X main.commit=$COMMIT -X main.date=$DATE" pgwatch2.go prom.go patroni.go logparse.go
  else
    echo "running (4): go build pgwatch2.go prom.go patroni.go logparse.go" # no  version info
    go build -gcflags "all=-N -l" pgwatch2.go prom.go patroni.go logparse.go
  fi
